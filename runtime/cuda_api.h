...
typedef enum CUstreamWaitValue_flags_enum {
  ...
  CU_STREAM_WAIT_VALUE_FLUSH =
      ...
      << 30 /**< Follow the wait operation with a flush of outstanding remote
                    ...
                    satisfied, that write is guaranteed to be visible to
                 downstream
                    device work. The device is permitted to reorder remote
                 writes
                    internally. For example, this flag would be required if two
                    remote writes arrive in a defined order, the wait is
                 satisfied
                    by the second write, and downstream work needs to observe
                 the
...
typedef enum CUdevice_attribute_enum {
  ...
  CU_DEVICE_ATTRIBUTE_VIRTUAL_ADDRESS_MANAGEMENT_SUPPORTED =
      102, /**< Device supports virtual address management APIs like
...
typedef enum CUpointer_attribute_enum {
  ...
  CU_POINTER_ATTRIBUTE_ALLOWED_HANDLE_TYPES =
      14 /**< Bitmask of allowed ::CUmemAllocationHandleType for this allocation
          **/
} CUpointer_attribute;
 ...
 * supports virtual address management APIs like ::cuMemAddressReserve,
...
/**
 * \brief Create a shareable memory handle representing a memory allocation of a
 ...
 *
 * - Kernel nodes:
 *   - The function must not change (same restriction as
 * cuGraphExecKernelNodeSetParams())
 ...
 * - CU_GRAPH_EXEC_UPDATE_ERROR_FUNCTION_CHANGED if the func field of a kernel
 * changed, in which case \p hErrorNode_out is set to the node from \p hGraph
